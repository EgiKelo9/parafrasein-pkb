...
class IndonesianTextPreprocessor:
    def __init__(self, tokenizer_name="Wikidepia/IndoT5-base-paraphrase"):
        ...
        Menggunakan tokenizer yang sama dengan teststreamlit.py
        
        Args:
            tokenizer_name: Nama model tokenizer yang akan digunakan
        """
        self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_name)
        self.max_length = 512  # Maksimum panjang sequence
        
        # Kamus kata-kata umum yang sering disingkat dalam bahasa Indonesia
    ...
    def clean_text(self, text):
        ...
        # Menghapus URL, email, dan nomor telepon
        text = self.url_pattern.sub(' ', text)
        text = self.email_pattern.sub(' ', text)
        text = self.phone_pattern.sub(' ', text)
        
        # Menghapus karakter khusus berlebihan, tapi pertahankan tanda baca penting
    ...
    def normalize_text(self, text):
        """
        Normalisasi teks termasuk expansion singkatan
    ...
    def calculate_similarity(self, text1, text2):
        """
        Menghitung similarity menggunakan metode yang sama dengan teststreamlit.py
        """
    ...
    def categorize_paraphrase(self, original, paraphrase):
        """
        Mengkategorikan parafrase berdasarkan similarity score
        Sama seperti di teststreamlit.py
        """
    ...
    def check_text_quality(self, text):
        """
        Memeriksa kualitas teks berdasarkan beberapa kriteria
        
        Args:
            text: Teks yang akan diperiksa
            
        Returns:
            dict: Skor kualitas dan detail
        """
    ...
    def preprocess_paraphrase_pair(self, original, paraphrase):
        """
        Preprocessing pasangan parafrase dengan kategori otomatis
        
        Args:
            original: Teks asli
            paraphrase: Teks parafrase
            
        Returns:
            dict: Pasangan teks yang telah diproses beserta metadatanya
        """
        # Clean dan normalize kedua teks
        ...
        # Tokenize untuk mendapatkan informasi tambahan
        ...
        return {
            ...
            'is_valid': (original_quality['quality_score'] >= 60 and 
                        ...
                        similarity_score < 0.95)  # Hindari duplikasi persis
        }

class ParaphraseDataset(Dataset):
    """
    Dataset class untuk training model parafrase
    Kompatibel dengan format teststreamlit.py
    """
    def __init__(self, data, tokenizer, max_length=512, category_filter=None):
        """
        Args:
            data: DataFrame berisi pasangan parafrase
            tokenizer: Tokenizer yang akan digunakan
            max_length: Panjang maksimum sequence
            category_filter: Filter kategori ('safe', 'balanced', 'creative')
        """
        self.data = data
        if category_filter:
    ...
    def __getitem__(self, idx):
        ...
        # Temperature mapping berdasarkan kategori (seperti di teststreamlit.py)
        temperature_map = {
            'safe': 0.7,
            'balanced': 1.0,
            'creative': 1.3
...
def tokenize_data_for_training(examples, tokenizer, max_length=512):
    """
    Tokenisasi data untuk training dengan format yang kompatibel
...
def create_data_splits(data, test_size=0.2, val_size=0.1, random_state=42):
    ...
    dengan mempertahankan distribusi kategori
    
    Args:
        data: DataFrame berisi data parafrase
        test_size: Proporsi data test
        val_size: Proporsi data validation
    ...
    try:
        ...
        # Split temp_data menjadi validation dan test
    ...
    # Tampilkan distribusi kategori per split
...
def prepare_training_data_like_streamlit(data, tokenizer):
    """
    Mempersiapkan data training dengan format yang sama seperti teststreamlit.py
    """
    train_data = []
...
if __name__ == "__main__":
    # Load data yang telah dikumpulkan
    try:
    ...
    # Initialize preprocessor dengan tokenizer yang sama seperti teststreamlit.py
    preprocessor = IndonesianTextPreprocessor(tokenizer_name="Wikidepia/IndoT5-base-paraphrase")
    
    # Preprocessing semua pasangan
    processed_pairs = []
    ...
    # Prepare training format seperti teststreamlit.py
    ...
    # Contoh membuat dataset untuk training
