...
# Import dari model architecture dan training pipeline
...
class ParafraseInApp:
    """
    Main application class yang mengintegrasikan UI teststreamlit.py 
    dengan model dari training_pipeline.py dan model_architecture.py
    """
    def __init__(self):
        # Initialize model menggunakan ParaphraseModel dari model_architecture.py
    ...
    def load_fine_tuned_model_if_exists(self):
        """
        Load model yang sudah di-fine-tune jika ada
        """
    ...
    def generate_paraphrases(self, text, style="safe"):
        """
        Generate parafrase menggunakan model dari model_architecture.py
        """
        if not text.strip():
    ...
    def get_word_mappings(self, original_text, paraphrased_text):
        """
        Get word mappings untuk display
        """
    ...
    def save_preference(self, original_text, paraphrase, style):
        """
        Simpan preferensi user menggunakan PreferenceTracker (tanpa rating)
        """
        ...
        # Save to model's preference file too
    ...
    def fine_tune_model(self, epochs=3, learning_rate=5e-5):
        """
        Fine-tune model menggunakan SimplifiedParaphraseTrainer
        """
        try:
            # Initialize trainer jika belum ada
            if not self.trainer:
                self.trainer = SimplifiedParaphraseTrainer(self.config)
            
            # Update user preferences untuk training
            ...
            # Update config dengan parameter baru
...
def load_parafrase_app():
    """
    Load application dengan caching untuk performa
    """
    app = ParafraseInApp()
...
def display_word_changes(original_text, paraphrased_text, app):
    """
    Display word changes section dengan styling yang sama seperti teststreamlit.py
    """
    ...
    # Display word changes in a grid format
...
def display_preference_statistics(app):
    """
    Display statistik preferensi user
    """
...
def main():
    ...
    # Custom CSS styling yang sama dengan teststreamlit.py
    st.markdown("""
    <style>
    .main-header {
        font-size: 3rem;
    ...
    with st.sidebar:
        ...
        if st.button("🚀 Mulai Fine-tuning"):
            if total_prefs >= 5:  # Minimum preferences needed
        ...
        if st.button("🗑️ Clear All Preferences"):
            ...
            # Clear and save empty preferences to JSON file
            app.preference_tracker.save_preferences()
            
            # Clear model's user preferences
            app.paraphrase_model.user_preferences = []
            
            # Also clear the model's preference file if it exists
            try:
