...
class SimplifiedParaphraseTrainer:
    """
    Trainer class yang disederhanakan berdasarkan teststreamlit.py
    Menggunakan arsitektur yang sudah terbukti bekerja
    """
    def __init__(self, config):
        ...
        # Training parameters - menggunakan nilai yang sama dengan teststreamlit.py
        ...
        # Model parameters - menggunakan model yang sama dengan teststreamlit.py
        self.model_name = self.config.get('model_name', 'Wikidepia/IndoT5-base-paraphrase')
        
        # Initialize model menggunakan arsitektur dari model_architecture.py
        self.model = None
        self.tokenizer = None
        ...
        # User preferences untuk fine-tuning (seperti di teststreamlit.py)
        self.user_preferences = []
    
    def setup_model(self):
        """
        Set up model menggunakan arsitektur yang sama dengan teststreamlit.py
        """
        logger.info("Setting up model using ParaphraseModel architecture...")
        
        # Gunakan ParaphraseModel dari model_architecture.py
    ...
    def setup_data_from_preferences(self, preferences_file="user_preferences.json"):
        """
        Setup data dari user preferences seperti di teststreamlit.py
        
        Args:
            preferences_file: Path ke file preferensi user
        """
        ...
        try:
            ...
            ] * 10  # Duplicate untuk testing
        
        if not self.user_preferences:
            logger.error("No training data available")
            return
        
        # Prepare training data seperti di teststreamlit.py
        train_data = []
        ...
        # Convert to DataFrame dan split
        ...
        # Create datasets menggunakan format teststreamlit.py
    ...
    def tokenize_data(self, examples):
        """
        Tokenisasi data menggunakan metode yang sama dengan teststreamlit.py
        """
    ...
    def train_with_transformers(self):
        """
        Training menggunakan Transformers Trainer seperti di teststreamlit.py
        """
        logger.info("Starting training with Transformers Trainer...")
        
        # Training arguments yang sama dengan teststreamlit.py
        training_args = TrainingArguments(
            ...
            fp16=self.use_mixed_precision,  # Mixed precision
            max_steps=self.max_steps_per_epoch * self.max_epochs  # Limit total steps
        )
        ...
        try:
            ...
            # Save model seperti di teststreamlit.py
    ...
    def fine_tune_from_preferences(self, preferences_file="user_preferences.json"):
        """
        Fine-tune model dari user preferences seperti di teststreamlit.py
        """
    ...
    def generate_paraphrase_test(self, text, style="balanced"):
        """
        Test generation menggunakan model yang sudah di-train
        """
        if not self.model or not self.tokenizer:
            return "Model not loaded"
        
        # Gunakan metode yang sama dengan teststreamlit.py
...
class LegacyParaphraseTrainer:
    """
    Trainer class original yang kompleks (sebagai backup)
    """
    def __init__(self, config):
...
def create_default_config():
    """
    Buat konfigurasi default yang sesuai dengan teststreamlit.py
    """
    return {
        "max_epochs": 3,
        "batch_size": 2,
...
def main():
    ...
    if args.mode == "train":
        # Fine-tune from user preferences
